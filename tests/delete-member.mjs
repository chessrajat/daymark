...
try{
 ...
 const form=new FormData();form.set('file',new Blob(['Keep attachment']),'keep.txt');assert.equal((await request('/api/tasks/'+t,{method:'POST',body:form})).status,200);
 ...
 const events=await(await request('/api/tasks/'+t)).json();assert.ok(events.some(e=>e.message==='Keep history'));assert.ok(events.some(e=>e.message.includes('Independent test member')&&e.message.includes('deleted')));const attachment=events.find(e=>e.attachment_id);assert.equal(await(await request('/api/attachments/'+attachment.attachment_id)).text(),'Keep attachment');
